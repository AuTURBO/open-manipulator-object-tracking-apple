...
bool OM_EXAMPLE::setTaskSpacePath(std::vector<double> kinematics_pose, double path_time)
{
  ...
  ROS_INFO("-- x( %.3lf) y( %.3lf) z( %.3lf,) \n", srv.request.kinematics_pose.pose.position.x,\
  	srv.request.kinematics_pose.pose.position.y,srv.request.kinematics_pose.pose.position.z);
  srv.request.path_time = path_time;
